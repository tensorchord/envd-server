...
func NewProvisioner(client kubernetes.Interface,
	namespace, imagePullSecretName string, resourceQuotaEnabled bool) runtime.Provisioner {
	p := &generalProvisioner{
		...
		logger: logrus.WithFields(logrus.Fields{
			...
			"resource-quota-enabled": resourceQuotaEnabled,
		}),
	}
	if imagePullSecretName != "" {
		p.imagePullSecretName = &imagePullSecretName
	}
	p.resourceQuotaEnabled = resourceQuotaEnabled
	return p
}
